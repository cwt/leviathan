...
Zig development is funded via [Zig Software Foundation](/zsf/), a 501(c)(3) non-profit organization. Please consider a recurring donation so that we can offer more billable hours to our core team members. This is the most straightforward way to accelerate the project along the [Roadmap](#Roadmap) to 1.0.
...
Debug compilation is 5 times faster with Zig's [x86 Backend](#x86-Backend) selected by default; the work-in-progress [aarch64 Backend](#aarch64-Backend) hot on its heels. Meanwhile, the [Writergate](#Writergate) scandal, along with a slew of [Language Changes](#Language-Changes) and [Standard Library](#Standard-Library) cuts, rocks the boat with tidal waves of breaking API changes; the harbinger of [async/await resurrection](https://kristoff.it/blog/zig-new-async-io/); the last bastion defending language stabilization.
...
-   [Language Changes](#Language-Changes)
    -   [usingnamespace Removed](#usingnamespace-Removed)
        -   [Use Case: Conditional Inclusion](#Use-Case-Conditional-Inclusion)
        -   [Use Case: Implementation Switching](#Use-Case-Implementation-Switching)
        -   [Use Case: Mixins](#Use-Case-Mixins)
    -   [async and await keywords removed](#async-and-await-keywords-removed)
    -   [switch on non-exhaustive enums](#switch-on-non-exhaustive-enums)
    -   [Allow more operators on bool vectors](#Allow-more-operators-on-bool-vectors)
    -   [Inline Assembly: Typed Clobbers](#Inline-Assembly-Typed-Clobbers)
    -   [Allow @ptrCast Single-Item Pointer to Slice](#Allow-ptrCast-Single-Item-Pointer-to-Slice)
    -   [New Rules for Arithmetic on undefined](#New-Rules-for-Arithmetic-on-undefined)
    -   [Error on Lossy Coercion from Int to Float](#Error-on-Lossy-Coercion-from-Int-to-Float)
-   [Standard Library](#Standard-Library)
    -   [Writergate](#Writergate)
        -   [Motivation](#Motivation)
        -   [Adapter API](#Adapter-API)
        -   [New std.Io.Writer and std.Io.Reader API](#New-stdIoWriter-and-stdIoReader-API)
        -   [std.fs.File.Reader and std.fs.File.Writer](#stdfsFileReader-and-stdfsFileWriter)
        -   [Upgrading std.io.getStdOut().writer().print()](#Upgrading-stdiogetStdOutwriterprint)
        -   [reworked std.compress.flate](#reworked-stdcompressflate)
        -   [CountingWriter Deleted](#CountingWriter-Deleted)
        -   [BufferedWriter Deleted](#BufferedWriter-Deleted)
    -   ["{f}" Required to Call format Methods](#f-Required-to-Call-format-Methods)
    -   [Format Methods No Longer Have Format Strings or Options](#Format-Methods-No-Longer-Have-Format-Strings-or-Options)
    -   [Formatted Printing No Longer Deals with Unicode](#Formatted-Printing-No-Longer-Deals-with-Unicode)
    -   [New Formatted Printing Specifiers](#New-Formatted-Printing-Specifiers)
    -   [De-Genericify Linked Lists](#De-Genericify-Linked-Lists)
    -   [std.Progress supports progress bar escape codes](#stdProgress-supports-progress-bar-escape-codes)
    -   [HTTP Client and Server](#HTTP-Client-and-Server)
    -   [TLS Client](#TLS-Client)
    -   [ArrayList: make unmanaged the default](#ArrayList-make-unmanaged-the-default)
    -   [Ring Buffers](#Ring-Buffers)
    -   [Removal of BoundedArray](#Removal-of-BoundedArray)
    -   [Deletions and Deprecations](#Deletions-and-Deprecations)
-   [Build System](#Build-System)
    -   [Removed Deprecated Implicit Root Module](#Removed-Deprecated-Implicit-Root-Module)
    -   [macOS File System Watching](#macOS-File-System-Watching)
    -   [Web Interface and Time Report](#Web-Interface-and-Time-Report)
-   [Compiler](#Compiler)
    -   [x86 Backend](#x86-Backend)
    -   [aarch64 Backend](#aarch64-Backend)
    -   [Incremental Compilation](#Incremental-Compilation)
    -   [Threaded Codegen](#Threaded-Codegen)
    -   [Allow configuring UBSan mode at the module level](#Allow-configuring-UBSan-mode-at-the-module-level)
    -   [Compile Tests to Object File](#Compile-Tests-to-Object-File)
    -   [Zig Init](#Zig-Init)
-   [Linker](#Linker)
-   [Fuzzer](#Fuzzer)
-   [Bug Fixes](#Bug-Fixes)
...
-   [Toolchain](#Toolchain)
    -   [LLVM 20](#LLVM-20)
    -   [Support dynamically-linked FreeBSD libc when cross-compiling](#Support-dynamically-linked-FreeBSD-libc-when-cross-compiling)
    -   [Support dynamically-linked NetBSD libc when cross-compiling](#Support-dynamically-linked-NetBSD-libc-when-cross-compiling)
    -   [glibc 2.42](#glibc-242)
        -   [Allow linking native glibc statically](#Allow-linking-native-glibc-statically)
    -   [MinGW-w64](#MinGW-w64)
    -   [zig libc](#zig-libc)
    -   [zig cc](#zig-cc)
    -   [zig objcopy regressed](#zig-objcopy-regressed)
-   [Roadmap](#Roadmap)
    -   [I/O as an Interface](#IO-as-an-Interface)
-   [Thank You Contributors!](#Thank-You-Contributors)
-   [Thank You Sponsors!](#Thank-You-Sponsors)

## [Target Support](#toc-Target-Support) [§](#Target-Support)

Zig supports a wide range of architectures and operating systems. The [Support Table](#Support-Table) and [Additional Platforms](#Additional-Platforms) sections cover the targets that Zig can build programs for, while the [zig-bootstrap README](https://github.com/ziglang/zig-bootstrap/blob/master/README.md#supported-triples) covers the targets that the Zig compiler itself can be easily cross-compiled to run on.
...
### [usingnamespace Removed](#toc-usingnamespace-Removed) [§](#usingnamespace-Removed)
...
Finally, removal of this feature makes [Incremental Compilation](#Incremental-Compilation) fundamentally simpler.

#### [Use Case: Conditional Inclusion](#toc-Use-Case-Conditional-Inclusion) [§](#Use-Case-Conditional-Inclusion)
...
#### [Use Case: Implementation Switching](#toc-Use-Case-Implementation-Switching) [§](#Use-Case-Implementation-Switching)
...
#### [Use Case: Mixins](#toc-Use-Case-Mixins) [§](#Use-Case-Mixins)
...
### [async and await keywords removed](#toc-async-and-await-keywords-removed) [§](#async-and-await-keywords-removed)

Also removed `@frameSize`.

While `suspend`, `resume`, and other machinery might remain depending on [Proposal: stackless coroutines as low-level primitives](https://github.com/ziglang/zig/issues/23446), it is settled that there will not be async/await keywords in the language. Instead, they will be in the [Standard Library](#Standard-Library) as part of the [Io Interface](#IO-as-an-Interface).

### [switch on non-exhaustive enums](#toc-switch-on-non-exhaustive-enums) [§](#switch-on-non-exhaustive-enums)
...
### [Allow more operators on bool vectors](#toc-Allow-more-operators-on-bool-vectors) [§](#Allow-more-operators-on-bool-vectors)

Allow binary not, binary and, binary or, binary xor, and boolean not operators on vectors of `bool`.

### [Inline Assembly: Typed Clobbers](#toc-Inline-Assembly-Typed-Clobbers) [§](#Inline-Assembly-Typed-Clobbers)
...
### [Allow @ptrCast Single-Item Pointer to Slice](#toc-Allow-ptrCast-Single-Item-Pointer-to-Slice) [§](#Allow-ptrCast-Single-Item-Pointer-to-Slice)
...
### [New Rules for Arithmetic on undefined](#toc-New-Rules-for-Arithmetic-on-undefined) [§](#New-Rules-for-Arithmetic-on-undefined)
...
### [Error on Lossy Coercion from Int to Float](#toc-Error-on-Lossy-Coercion-from-Int-to-Float) [§](#Error-on-Lossy-Coercion-from-Int-to-Float)
...
### [Writergate](#toc-Writergate) [§](#Writergate)
...
#### [Motivation](#toc-Motivation) [§](#Motivation)
...
#### [Adapter API](#toc-Adapter-API) [§](#Adapter-API)
...
#### [New std.Io.Writer and std.Io.Reader API](#toc-New-stdIoWriter-and-stdIoReader-API) [§](#New-stdIoWriter-and-stdIoReader-API)
...
#### [std.fs.File.Reader and std.fs.File.Writer](#toc-stdfsFileReader-and-stdfsFileWriter) [§](#stdfsFileReader-and-stdfsFileWriter)
...
#### [Upgrading std.io.getStdOut().writer().print()](#toc-Upgrading-stdiogetStdOutwriterprint) [§](#Upgrading-stdiogetStdOutwriterprint)
...
#### [reworked std.compress.flate](#toc-reworked-stdcompressflate) [§](#reworked-stdcompressflate)
...
It will be nice to get deflate back into the Zig standard library, but for now, progressing the language takes priority over progressing the standard library, and this change is on the path towards locking in the final language design with respect to [I/O as an Interface](#IO-as-an-Interface).
...
#### [CountingWriter Deleted](#toc-CountingWriter-Deleted) [§](#CountingWriter-Deleted)
...
#### [BufferedWriter Deleted](#toc-BufferedWriter-Deleted) [§](#BufferedWriter-Deleted)
...
### ["{f}" Required to Call format Methods](#toc-f-Required-to-Call-format-Methods) [§](#f-Required-to-Call-format-Methods)
...
### [Format Methods No Longer Have Format Strings or Options](#toc-Format-Methods-No-Longer-Have-Format-Strings-or-Options) [§](#Format-Methods-No-Longer-Have-Format-Strings-or-Options)
...
### [Formatted Printing No Longer Deals with Unicode](#toc-Formatted-Printing-No-Longer-Deals-with-Unicode) [§](#Formatted-Printing-No-Longer-Deals-with-Unicode)
...
### [New Formatted Printing Specifiers](#toc-New-Formatted-Printing-Specifiers) [§](#New-Formatted-Printing-Specifiers)
...
### [De-Genericify Linked Lists](#toc-De-Genericify-Linked-Lists) [§](#De-Genericify-Linked-Lists)
...
### [std.Progress supports progress bar escape codes](#toc-stdProgress-supports-progress-bar-escape-codes) [§](#stdProgress-supports-progress-bar-escape-codes)
...
It integrates with `--watch` in the [Build System](#Build-System) to set error state when failures occur, and clear it when they are fixed, also to clear progress when waiting for user input.
...
### [HTTP Client and Server](#toc-HTTP-Client-and-Server) [§](#HTTP-Client-and-Server)
...
### [TLS Client](#toc-TLS-Client) [§](#TLS-Client)

`std.crypto.tls.Client` no longer depends on `std.net` or `std.fs`. Instead, it only depends on `std.Io.Reader` and `std.Io.Writer`.

### [ArrayList: make unmanaged the default](#toc-ArrayList-make-unmanaged-the-default) [§](#ArrayList-make-unmanaged-the-default)
...
### [Ring Buffers](#toc-Ring-Buffers) [§](#Ring-Buffers)
...
Furthermore, most of its real-world use cases are subsumed by [New std.Io.Writer and std.Io.Reader API](#New-stdIoWriter-and-stdIoReader-API) which are both ring buffers.

Similarly, `std.RingBuffer` is removed since it was only used by the zstd implementation which has been upgraded to use [New std.Io.Writer and std.Io.Reader API](#New-stdIoWriter-and-stdIoReader-API).

There was also `std.compress.flate.CircularBuffer` which was internal to the flate implementation; now deleted.

There was also one each in [HTTP Client and Server](#HTTP-Client-and-Server) - again deleted in favor of [New std.Io.Writer and std.Io.Reader API](#New-stdIoWriter-and-stdIoReader-API).
...
### [Removal of BoundedArray](#toc-Removal-of-BoundedArray) [§](#Removal-of-BoundedArray)
...
### [Deletions and Deprecations](#toc-Deletions-and-Deprecations) [§](#Deletions-and-Deprecations)
...
### [Removed Deprecated Implicit Root Module](#toc-Removed-Deprecated-Implicit-Root-Module) [§](#Removed-Deprecated-Implicit-Root-Module)
...
src/download/0.15.1/release-notes/deprecated-addExecutable.zig:4:10: error: no field named 'root\_source\_file' in struct 'Build.ExecutableOptions'
        .root\_source\_file = b.path("src/main.zig"),
         ^~~~~~~~~~~~~~~~
/home/ci/deps/zig-x86\_64-linux-0.15.2/lib/std/Build.zig:771:31: note: struct declared here
...
### [macOS File System Watching](#toc-macOS-File-System-Watching) [§](#macOS-File-System-Watching)
...
So, if you were avoiding `--watch` in previous Zig versions due to the macOS bug, you can now use it safely. This is particularly useful if you are interested in trying [Incremental Compilation](#Incremental-Compilation), since the typical way to use that feature today involves passing the flags `--watch -fincremental` to `zig build`.

### [Web Interface and Time Report](#toc-Web-Interface-and-Time-Report) [§](#Web-Interface-and-Time-Report)
...
The web interface is, by itself, relatively uninteresting: it merely shows the list of build steps and information about which are in progress, and has a button to manually trigger a rebuild (hence giving a possible alternative to the `zig build --watch` workflow). If `--fuzz` is passed to `zig build`, it also exposes the [Fuzzer](#Fuzzer) interface, which is mostly unchanged from 0.14.0.
...
### [x86 Backend](#toc-x86-Backend) [§](#x86-Backend)
...
More specifically, this backend is now the default when targeting x86\_64 in Debug mode, except on NetBSD, OpenBSD, and Windows, where the LLVM backend is still the default due to [Linker](#Linker) deficiencies.

When this backend is selected, you will begin to reap the benefits of the investments the Zig project has made over the past few years. Compilation time is *significantly* improved—around a 5x decrease compared to LLVM in most cases. **This is only the beginning;** the self-hosted x86\_64 backend has been built to support [Incremental Compilation](#Incremental-Compilation), which will result in another extreme speed-up when it is stable enough to be enabled by default. Fast compilation is a key goal of the Zig project, and one we have been quietly making progress on for years—this release sees those efforts starting to come to fruition.
...
### [Threaded Codegen](#toc-Threaded-Codegen) [§](#Threaded-Codegen)
...
Compared to 0.14.0, this typically leads to another performance boost when using self-hosted backends such as the [x86 Backend](#x86-Backend). The improvement in wall-clock time varies from relatively insignificant to upwards of 50% depending on the specific code being compiled. However, as one real-world data point, building the Zig compiler using its own x86\_64 backend got 27% faster on one system from this change, with the wall-clock time going from 13.8s to 10.0s.
...
### [Allow configuring UBSan mode at the module level](#toc-Allow-configuring-UBSan-mode-at-the-module-level) [§](#Allow-configuring-UBSan-mode-at-the-module-level)
...
For [zig cc](#zig-cc), in addition to the existing `-fsanitize=undefined`, `-fsanitize-trap=undefined` is now also understood and is generally equivalent to `-fsanitize-c=trap` for `zig build-exe`.
...
### [Compile Tests to Object File](#toc-Compile-Tests-to-Object-File) [§](#Compile-Tests-to-Object-File)
...
### [Zig Init](#toc-Zig-Init) [§](#Zig-Init)
...
Zig's linker received only bug fixes and maintenance during this release cycle. However, it will be a key focus in the [next release cycle](#Roadmap) in order to improve [Incremental Compilation](#Incremental-Compilation).
...
### [Support dynamically-linked FreeBSD libc when cross-compiling](#toc-Support-dynamically-linked-FreeBSD-libc-when-cross-compiling) [§](#Support-dynamically-linked-FreeBSD-libc-when-cross-compiling)
...
### [Support dynamically-linked NetBSD libc when cross-compiling](#toc-Support-dynamically-linked-NetBSD-libc-when-cross-compiling) [§](#Support-dynamically-linked-NetBSD-libc-when-cross-compiling)
...
### [glibc 2.42](#toc-glibc-242) [§](#glibc-242)

glibc version 2.42 is now available when cross-compiling.

#### [Allow linking native glibc statically](#toc-Allow-linking-native-glibc-statically) [§](#Allow-linking-native-glibc-statically)
...
### [zig objcopy regressed](#toc-zig-objcopy-regressed) [§](#zig-objcopy-regressed)
...
-   Making the [aarch64 Backend](#aarch64-Backend) the default backend for debug mode.
-   Enhance [Linker](#Linker) implementations, eliminating dependency on [LLD](https://lld.llvm.org/) and supporting [Incremental Compilation](#Incremental-Compilation).
-   Enhance the integrated [Fuzzer](#Fuzzer) to be competitive with AFL and other state-of-the-art fuzzers.

### [I/O as an Interface](#toc-IO-as-an-Interface) [§](#IO-as-an-Interface)
...
## [Thank You Contributors!](#toc-Thank-You-Contributors) [§](#Thank-You-Contributors)
...
## [Thank You Sponsors!](#toc-Thank-You-Sponsors) [§](#Thank-You-Sponsors)
